...
public class BankBalance {
	...
	public BankBalance process(BankTransaction bankTransaction) {
		...
		if (this.amount.add(bankTransaction.getAmount()).compareTo(BigDecimal.ZERO) >= 0) {
			addLatestTransaction(transactionBuilder.state(BankTransactionState.APPROVED).build());
			this.amount = this.amount.add(bankTransaction.getAmount());
		} else {
			addLatestTransaction(transactionBuilder.state(BankTransactionState.REJECTED).build());
		}
		this.lastUpdate = bankTransaction.getTime();
		return this;
	}

	public BankBalance process(avro.tn.biltounsi.BankTransaction avroBbankTransaction) {
		BankTransaction bankTransaction = new BankTransaction(avroBbankTransaction);
		this.id = bankTransaction.getBalanceId();
		var transactionBuilder = bankTransaction.toBuilder();
		if (this.amount.add(bankTransaction.getAmount()).compareTo(BigDecimal.ZERO) >= 0) {
			addLatestTransaction(transactionBuilder.state(BankTransactionState.APPROVED).build());
